...
def simplex():
    _,n_variables,restricciones, objetivo,maxmin = ingresaProblema()
    nuevas_rest,holg_ex = convertirEstandar(restricciones)

    print(toString(n_variables, nuevas_rest, objetivo, maxmin))

    variables, renglones = crearTabla(restricciones,holg_ex,n_variables,objetivo)
    imprimirTabla(variables,renglones)
    positivos = False
    while positivos == False:
        ...
        #Pivotear
        renglones, err = nuevoTableau(renglones,pos_col, variables)
        ...
        positivos = todosPositivos(renglones["ro"])
...
def ingresaProblema():
    ...
    :return restricciones: (list) una de tamaño n_restricciones que contiene cada restricción representada como otra lista de coeficientes
    :return objetivo: (list) una lista con los coeficientes de la función objetivo
    :return relaciones: (list) una lista de tamaño n_restricciones que representan el tipo de restricción, siendo 1 para "=", 2 para "<=", y 3 para ">="
